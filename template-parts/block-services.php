...
$page_id = get_the_ID();
$page_prefix = is_404() ? 'options' : $page_id;
$services_block_blockcolor = is_404() ? get_field('services_block_blockcolor_404', $page_prefix) : get_field('services_block_blockcolor', $page_prefix); // Цвет фона блока
...
$services_block_title = get_field('services_block_title', $page_prefix);
$services_block_text = get_field('services_block_text', $page_prefix);
                ...
                if ($query_services->have_posts()) {
                    ...
                    while ($query_services->have_posts()):
                        ...
                        <li class="services-list__item services-item"
                            style="background-image:url( <?php echo $service_listing_icon['url']; ?> );" data-aos="fade-<?php
                                if ($index == 0 || $index == 3)
                                    echo 'right';
                                if ($index == 1)
                                    echo 'down';
